...
export function generateAccessToken(payload: UserPayload){
  const accessToken:string = sign({id: payload}, env.JWT_ACCESS_SECRET, {expiresIn:'30m'});
  return accessToken
}
export function generateRefreshToken(payload: UserPayload){
  const refreshToken:string = sign({id: payload}, env.JWT_REFRESH_SECRET, {expiresIn:'30d'});
  return refreshToken
}
